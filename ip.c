...
unsigned char my_mac[6]; // put your mac address here
unsigned char my_ip[4]; // put your ip address here
...
void forge_icmp_echo(struct icmp_packet * icmp )
{
	icmp->type = 8;
	icmp->code = 0;
	...
	int i;
	for (i=0; i<20; i++) icmp->payload[i] = i & 0xff; // a typical pattern. The and is done to not exceed the size
	
	icmp->checksum = htons(checksum((unsigned char *) icmp, 28)); // 28 = 8 + payloadsize	
}

void forge_ip(struct ip_datagram * ip, int payloadsize, char proto, unsigned int target_ip)
{
	...
	ip->flags_offset = 0; // original packet
	ip->time_to_live = 128;
	ip->protocol = proto; 
...
int main(int argc, char** argv)
{
	...
	if (argc == 2)
		*(unsigned int *)target_ip = inet_addr(argv[1]);
	...
	forge_icmp_echo(icmp);
	forge_ip(ip, 28, 1, *(unsigned int *) target_ip); // proto = 1 according to iana.org/assignments/protocol-numbers
	...
	printbuf(buffer, 62); // 14 for the eth header, 20 for the ip header, 8 for the icmp header, 20 for the icmp payload	
	printf("\n");
	
	t = sendto(s, buffer, 62, 0, (struct sockaddr *) &sll, len);
	...
	while(1) 
	{
		...
		if (ntohs(eth->upper_layer_protocol) == 0x0800)
			if (are_equal(ip->source_address, (unsigned int *) target_ip, 4)) // ip->source_address == *(unsigned int *) target_ip)
				if (ip->protocol == 1)
					if (icmp->type == 0 && ntohs(icmp->identifier) == 0x1234 && htons(icmp->sequence_number) == 1)
					{
						printbuf(buffer, 62); // print because we are sure this is our reply	
						break;
					}
	}
